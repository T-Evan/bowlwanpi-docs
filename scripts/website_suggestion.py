...
def generate_website_suggestion():
    """生成每日网站优化建议"""
    # 随机选择1-2个建议
    suggestions = random.sample(WEBSITE_SUGGESTIONS, min(2, len(WEBSITE_SUGGESTIONS)))
    
    result = []
    result.append("🌐 【每日网站优化建议】\n")
    ...
    result.append("---")
    result.append("💭 你觉得这个建议怎么样？明天见~ 🥣")
    
    return "\n".join(result)

if __name__ == "__main__":
    print(generate_website_suggestion())
